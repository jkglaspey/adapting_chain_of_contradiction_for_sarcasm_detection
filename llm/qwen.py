...
def start_conversation_with_qwen(iterable_list, filename: str):
    ...
    for idx, (label, item) in enumerate(iterable_list[(start_idx - 1):], start=start_idx):
        ...
        first_response = chat_with_qwen(updated_prompt)
        if debug:
            print(f"Qwen Response 1: {first_response}")

        # Step 2: Ask the second question, using the first response
        second_question = CoCPrompts[1]
        second_response = chat_with_qwen(second_question, previous_responses=[updated_prompt, first_response])
        if debug:
            print(f"Qwen Response 2: {second_response}")

        # Step 3: Ask the third question, using the second response
        third_question = CoCPrompts[2]
        third_response = chat_with_qwen(third_question, previous_responses=[updated_prompt, first_response, second_question, second_response])
        if debug:
            print(f"Qwen Response 3: {third_response}")
        ...
        try:
            ...
            fourth_response = chat_with_qwen(fourth_question, previous_responses=[updated_prompt, first_response, second_question, second_response, third_question, third_response])
            if debug:
                print(f"Qwen Response 4: {fourth_response}")
            try:
                res = is_sarcastic(fourth_response)
            except ValueError:
                res = -1  # Mark as -1 if LLM could not handle it
                print(f"Qwen could not format a response for {item}")
...
def chat_with_qwen(question, previous_responses=None):
    ...
    if previous_responses:
        ...
        for response in previous_responses:
            if flag:
                messages_history.append({"role": "user", "content": response})
            else:
...
def initialize_chat_with_qwen(iterable_list, filename: str):

    # Run the main logic
    TP, TN, FP, FN = start_conversation_with_qwen(iterable_list, filename)
